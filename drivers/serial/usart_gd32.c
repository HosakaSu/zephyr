...
static uint32_t usart_gd32_data_addr(const struct device *dev)
{
	const struct gd32_usart_config *const cfg = dev->config;

	return (uint32_t)(uintptr_t)&USART_DATA(cfg->reg);
}
				    ...
				    uint8_t *buf, size_t len)
{
	...
	struct gd32_usart_dma_data *dma_data;
	uint32_t usart_data_addr = usart_gd32_data_addr(dev);
	int ret;
	...
	if (dir == GD32_USART_DMA_TX) {
		...
		dma_data->block.dest_address = usart_data_addr;
		...
		dma_data->block.source_address = usart_data_addr;
...
#define GD32_USART_DMA_CONFIG_INIT(n)                                                              \
	.dma_tx = GD32_USART_DMA_INIT(n, tx), .dma_rx = GD32_USART_DMA_INIT(n, rx),
#else
#define GD32_USART_DMA_CONFIG_INIT(n)
#endif /* CONFIG_UART_ASYNC_API */
...
#define GD32_USART_INIT(n)                                                                         \
	...
	static const struct gd32_usart_config usart_gd32_config_##n = {                            \
		...
		GD32_USART_DMA_CONFIG_INIT(n) GD32_USART_IRQ_HANDLER_FUNC_INIT(n)};                \
